...
`main` is the deploy branch — verified 2026-08-30, by pushing to `main` alone
and watching the live version change.

**Verify the deploy; never infer it from a successful push.** On 2026-08-30
Pages was found still serving `claude/local-events-discovery-vfjgwr`, so `main`
sat five commits ahead while the live site spent two days serving a Places page
whose every tap threw. The push succeeded, CI went green, and nothing anywhere
said the site had not moved. The setting is fixed now; the check stays, because
...
Give it a minute or two, and re-check rather than assuming. Do not report a
refresh as live until that `curl` returns the new version — a green CI run says
the data is valid, not that anyone can see it.
